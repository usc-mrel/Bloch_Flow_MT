% demo_figure3_PASL_high_perfusion_parameter_sweep.m
...
F      = 0.6;         % brain blood flow [mL/g/min]
...
alpha  = 0.98;        % labeling efficiency for PASL
...
for k = 1:dt_length
    ...
    for idx = 1:nr_elements
        ...
        %------------------------------------------------------------------
        % Calculate the pulsed ASL signal using the GKM
        %------------------------------------------------------------------
        PASL_analytic = calculate_GKM_pulsed_ASL_signal(t, F, lambda, m0b, alpha, T1, T1b, t_labeling, TD, TW);

        %------------------------------------------------------------------
        % Calculate the pulsed ASL bolus signal s(t)
        %------------------------------------------------------------------
        pulsed_ASL_bolus = calculate_pulsed_ASL_bolus(t, F, lambda, m0, alpha0, T1b, t_labeling, TD, TW * ones(nr_labeling,1), labeling_on); % [M0 / msec]
        ...
        [mx2,my2,mz2] = bloch_flow_mex(b1, gr, taus, zetas, T1, T2, 0, 0, 2, 0, 0, 1, F, lambda, m0, pulsed_ASL_bolus);
        PASL_numeric = mz1 - mz2; % control - labeled
        ...
        %------------------------------------------------------------------
        max_deviation(idx,k) = max(PASL_analytic - PASL_numeric);

        %------------------------------------------------------------------
        % Calculate the normalized root-mean-square error
        %------------------------------------------------------------------
        nrmse(idx,k) = norm(PASL_analytic - PASL_numeric) / norm(PASL_analytic);
...
%% Save results
save('PASL_high_perfusion_parameter_sweep', ...
